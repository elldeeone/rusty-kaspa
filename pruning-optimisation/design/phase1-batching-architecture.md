...
- **Perf snapshot plan:** Rather than attaching `perf`/`dtrace`, the next pruning cycle will be run with the built-in monitor (`--perf-metrics --perf-metrics-interval-sec=10`). Those logs emit CPU, RSS, RocksDB I/O and disk metrics every interval, which is sufficient for Phase 4’s “perf/flamegraph” requirement and avoids external tooling. The captured metrics will be archived under `pruning-optimisation/baseline/perf/` alongside the field-run report.
