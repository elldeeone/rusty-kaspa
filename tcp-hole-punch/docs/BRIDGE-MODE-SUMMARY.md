...
- Roles: `public` advertises relay service/port and uses the existing libp2p inbound split; `private/auto` do not advertise relay and cap libp2p inbound peers (default 8) while leaving TCP inbound unchanged. Auto currently resolves to private unless a helper listen address is set.
